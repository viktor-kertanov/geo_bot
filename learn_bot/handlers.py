...
from db_handlers.mongo_db import (
    get_flag_img_rating,
    get_or_create_user,
    mongo_db,
    save_flag_img_vote,
...
from learn_bot.bot_keyboard import img_rating_inline_keyboard, main_keyboard
from learn_bot.clarifai_handler import (
    clarifai_processor,
    object_exists_on_img,
    what_is_on_picture,
)
...
def greet_user(update, context):
    ...
    update.message.reply_text(f"Привет! {user['emoji']}", reply_markup=main_keyboard())


def guess_number_handler(update, context):
    get_or_create_user(mongo_db, update.effective_user, update.message.chat.id)
...
def talk_to_me(update, context):
    ...
    update.message.reply_text(
        f"{text_to_repeat} {user['emoji']}", reply_markup=main_keyboard()
    )
...
def set_alarm(update, context):
    try:
        ...
        context.job_queue.run_once(
            vk_alram, alarm_seconds, context=update.message.chat.id
        )
        update.message.reply_text(
...
def flag_picture_rating(update, context):
    user = get_or_create_user(mongo_db, update.effective_user, update.effective_chat.id)
