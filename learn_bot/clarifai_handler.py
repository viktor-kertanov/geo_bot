    ...
    if status_code == status_code_pb2.SUCCESS:
        ...
        for concept in response.outputs[0].data.concepts:
            if concept.name == object and concept.value >= min_confidence:
                print(f'We have found {object}, our confidence is: {concept.value:.3%}')
                return True
    else:
...
def what_is_on_picture(response, min_confidence):
    status_code = response.status.code
    if status_code == status_code_pb2.SUCCESS:
        print("Recognition was successful. Let's find out what we have on our pictures")

        found_elements = [concept for concept in response.outputs[0].data.concepts if concept.value >= min_confidence]
        
        for el_idx, el in enumerate(found_elements, start=1):
            print(f"{el_idx}. What: '{el.name.upper()}' ::: Confidence: {el.value: .2%}")
...
def process_many_images(images: list[str], min_confidence):
    ...
    for file in files_aux:
        ...
        found_names = what_is_on_picture(ai_response, min_confidence=min_confidence)
        found.append(found_names)
    
    return found

if __name__ == '__main__':
    files_aux = ['data/country_images/afghanistan_position.jpeg','data/country_images/albania_flag.jpeg']
