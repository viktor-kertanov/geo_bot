"""There are functions in this module that were used for one-time enrichment of the db.
This is closely connected with parsing data from Wiki
...
def enrich_iso_db_with_emoji(db):
    ...
    for c_idx, country in enumerate(db_data, start=1):
        ...
        if is_emoji(emoji):
            logger.info(f"{c_idx}. Adding emoji {emoji} for {name}.")
            db.iso_country_data.update_one(
                {"_id": country["_id"]}, {"$set": {"emoji": emoji}}
            )
        else:
            logger.info(f"{c_idx}. Skpping emoji for {name}. Add manually.")

    return db_data


def add_emoji_manually(db):
    """Find blank flag emojis in ISO country data db and paste manually"""
    blank_emojis = db.iso_country_data.find({"emoji": None})
    for c_idx, country in enumerate(blank_emojis, start=1):
        name = country["country_name"]
        # getting user input
        manual_emoji = input(
            f'Please insert emoji for {name} below. {country["country_page_url"]}\n'
        ).strip()

        # inserting to db if emoji
        if is_emoji(manual_emoji):
            logger.info(f"{c_idx}. Inserting emoji {manual_emoji} for {name}.")
            db.iso_country_data.update_one(
                {"_id": country["_id"]}, {"$set": {"emoji": manual_emoji}}
            )
        else:
            logger.info(f"{c_idx}. It's not emogji [{manual_emoji}]. Skipping {name}.")
    return blank_emojis


def enrich_with_region_data(db, region_data_filename, update_region_data=False):
    """Obtaining the region data in different languages,
    adding to the db update_region_data.
    bool if True than the data is overwritten in mongo db.
    Skip if False
    """
    ...
    for c_idx, country in enumerate(all_countries, start=1):
        ...
        for lang in region_data:
            # new field for a country is only created for the first language
            try:
                data_row = region_data[lang][numeric_code]
            except KeyError:
                logger.info("We have encountered the problem")
                logger.info(
                    f"No data in the source on {country_name} :: {numeric_code} : {lang}"
                )
                continue
        ...
        if "region_data" not in country or update_region_data:
            logger.info(f"{c_idx}. {country_name} :: {numeric_code}")
            db.iso_country_data.update_one(
                {"_id": country["_id"]}, {"$set": {"region_data": country_regions_dict}}
            )


def enrich_db_with_flag_position_url(db):
    countries = db.iso_country_data.find(
        {"$or": [{"country_flag_url": None}, {"country_position_url": None}]}
    )
    for c_idx, country in enumerate(countries, start=1):
        ...
        if flag_img:
            db.iso_country_data.update_one(
                {"_id": country["_id"]}, {"$set": {"country_flag_url": flag_img}}
            )
            logger.info(
                f'{c_idx}. {country["country_name"]}:: successfully added flag: {flag_img}'
            )
        else:
            logger.info(f'{c_idx}. {country["country_name"]}:: SKIP flag')
        if position_img:
            db.iso_country_data.update_one(
                {"_id": country["_id"]},
            ...
            logger.info(
                f'{c_idx}. {country["country_name"]}:: successfully added position: {position_img}'
            )
        else:
            logger.info(f'{c_idx}. {country["country_name"]}:: SKIP position')
...
def enrich_with_ru_wiki_article(db):
    ...
    for c_idx, country in enumerate(countries, start=1):
        ...
        ru_wiki_article = soup.select_one("li.interlanguage-link.interwiki-ru a").get(
            "href", None
        )
        if ru_wiki_article:
            db.iso_country_data.update_one(
                {"_id": country["_id"]}, {"$set": {"ru_wiki_article": ru_wiki_article}}
            )
        else:
            logger.info(f'{c_idx}. No wiki article for {country["country_name"]}.')
...
def enrich_with_ru_capital(db):
    ...
    for c_idx, country in enumerate(countries, start=1):
        ...
        db.iso_country_data.update_one(
            {"_id": country["_id"]}, {"$set": {"capital.russian": capital_data_row}}
        )
        ...
        for data_el in info_data:
            ...
            if data_row == "Столица":
                ...
                capital_data_row = {
                    "capital_name": capital_ru,
                    "capital_wiki_article_url": capital_url,
                }
                db.iso_country_data.update_one(
                    {"_id": country["_id"]},
                    {"$set": {"capital.russian": capital_data_row}},
                )
                continue
...
def enrich_with_eng_capital(db):
    ...
    for c_idx, country in enumerate(countries, start=1):
        capital_data_row = {"capital_name": "", "capital_wiki_article_url": ""}
        db.iso_country_data.update_one(
            {"_id": country["_id"]}, {"$set": {"capital.english": capital_data_row}}
        )
        ...
        for data_el in info_data:
            ...
            if data_row and "capital" in data_row:
                ...
                capital_data_row = {
                    "capital_name": capital_eng,
                    "capital_wiki_article_url": capital_url,
                }
                logger.info(
                    f'{c_idx}. Updating {country["country_name"]} with english capital data: {capital_eng} :: {capital_url}.'
                )
                db.iso_country_data.update_one(
                    {"_id": country["_id"]},
                    {"$set": {"capital.english": capital_data_row}},
                )
                continue
...
def download_position_images(db):
    ...
    countries = db.iso_country_data.find(
        {"iso_alpha_3_code": {"$nin": alpha_3_already_have}}
    )
...
def enrich_with_game_region(db):
    ...
    for c_idx, country in enumerate(countries, start=1):
        if (
            ...
            or country["country_name"] == "Taiwan, Province of China"
        ):
            continue
        try:
            country["region_data"]["english"]["region_3"]
                ...
                {"_id": country["_id"]},
                {
                    "$set": {
                        f"region_data.{lang}.game_region": (
                            lambda x: aux_region["region_3"]
                            if x
                            else aux_region["region_2"]
                        )(region_3_exists)
                    }
                },
            )
