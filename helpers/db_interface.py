...
def add_country(country_tuple_to_insert) -> None:
    with DatabaseCursor(GEO_DB) as cursor:
        ...
        VALUES (?,?,?,?,?,?)""", country_tuple_to_insert)
...
def absent_images() -> Dict:
    """
    Getting the total list of files that needed to be downloaded into a db and then converted to a readable format
    to machine
    """
    with DatabaseCursor(GEO_DB) as cursor:
        absent_flags = cursor.execute("""SELECT country_flag FROM countries WHERE country_flag_bytes IS NULL LIMIT 3;""").fetchall()
        absent_positions = cursor.execute("""SELECT position_on_map_image FROM countries WHERE position_on_map_image_bytes IS NULL LIMIT 3;""").fetchall()
        absent_flags = [f["country_flag"] for f in absent_flags]
        absent_positions = [p["position_on_map_image"] for p in absent_positions]
...
def country_title_get_db_row(country_title: str) -> Dict:
    with DatabaseCursor(GEO_DB) as cursor:
        ...
        WHERE 
        country_title LIKE ? OR country_eng_title LIKE ?;""", (f"%{country_title}%",f"%{country_title}%"))
        return dict(country_row.fetchone())


def write_image_to_db(country_title: str, db_col_name: str, image_bytes: bytes) -> None:
    with DatabaseCursor(GEO_DB) as cursor:
        cursor.execute(f"""
        UPDATE countries
        SET {db_col_name} = ? 
        ...
        print(f"{country_title}::{db_col_name}, len(image_bytes): {len(image_bytes)}, type(image_bytes): {type(image_bytes)}")
